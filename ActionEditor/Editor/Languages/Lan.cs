...
namespace PKC.ActionEditor
{
    internal class Lan
    {
        ...
        public static readonly Dictionary<string, Type> AllLanguages =
        ...
        {
            var lan = EditorPrefs.GetString("PKC.Editor.Lan",string.Empty);
            var types = ReflectionTools.GetImplementationsOf(typeof(ILanguages));
            foreach (var t in types)
            {
                var nameAttt = (NameAttribute)t.GetCustomAttributes(typeof(NameAttribute), false).FirstOrDefault();
                var name = nameAttt != null ? nameAttt.name : t.Name.SplitCamelCase();
                AllLanguages[name] = t;
        ...
        {
            _lan = key;
            EditorPrefs.SetString("PKC.Editor.Lan",key);
            ChangeLanguage();
        }
        ...
        private static void ChangeLanguage()
        {
            ...
            foreach (var field in fields)
            {
                ...
                {
                    Debug.LogError($"Failed to get value of field '{field.Name}': {ex.Message}");
                }
            }

            var defType = typeof(Lan);
            var defFields = defType.GetFields(BindingFlags.Public | BindingFlags.Static | BindingFlags.Instance);
            foreach (var field in defFields)
            {
                ...
                if (dictionary.TryGetValue(name, out var value))
                {
                    ...
                    {
                        Debug.LogError($"Failed to set value of field '{field.Name}': {ex.Message}");
                    }
                }
            }
        }
        ...
        //**********  Crate Window *********
        public static string CreateAssetType = "创建类型";
        public static string CreateAssetName = "时间轴名称";
